...
<Note>
**Default Model**: Claude-mem uses `deepseek/deepseek-v4-flash-0731` by default. Free OpenRouter models such as `xiaomi/mimo-v2-flash:free` remain available if you want zero-cost extraction.
</Note>
...
| `CLAUDE_MEM_OPENROUTER_MODEL` | string | `deepseek/deepseek-v4-flash-0731` | Model identifier (see list above) |
| `CLAUDE_MEM_OPENROUTER_PROVIDER` | string | — | Optional: OpenRouter endpoint slug to pin (e.g. `anthropic`). Empty uses automatic routing. |
...
6. Optionally set **OpenRouter Provider Slug** to pin a specific upstream endpoint

Settings are applied immediately—no restart required.

### Pinning an OpenRouter Provider

Leave `CLAUDE_MEM_OPENROUTER_PROVIDER` empty to keep OpenRouter's automatic routing. To force a single upstream, set it to the **endpoint slug** from the model's [OpenRouter providers table](https://openrouter.ai/models) — for example `anthropic`, `google-vertex`, or `amazon-bedrock`. That is not the same as the model id (`anthropic/claude-3.5-sonnet`).

When set, claude-mem sends:
...
If that provider cannot serve the model, the request fails instead of silently using another vendor. The slug is ignored (with a warning) when `CLAUDE_MEM_OPENROUTER_BASE_URL` points at a custom OpenAI-compatible gateway.

Server deployments can set the same value via the `CLAUDE_MEM_OPENROUTER_PROVIDER` environment variable.
...
```json
{
  ...
  "CLAUDE_MEM_OPENROUTER_MODEL": "deepseek/deepseek-v4-flash-0731"
}
```
...
When `CLAUDE_MEM_OPENROUTER_PROVIDER` is set, the body also includes `"provider": { "order": ["<slug>"], "allow_fallbacks": false }`.
...
<Tip>
**Recommendation**: The default OpenRouter model is `deepseek/deepseek-v4-flash-0731`. For zero-cost extraction, switch to a free model such as `xiaomi/mimo-v2-flash:free`. If you need higher quality or hit rate limits, switch to Claude or add OpenRouter credits for premium models.
</Tip>
